...
public:
    ...
    real DistanceSqToLine(const Point& A, const Point& B) const
    {
        return DistanceSq(ProjectionToLine(A, B));
    }

    real DistanceToLine(const Point& A, const Point& B) const
    {
        return std::sqrt(DistanceSqToLine(A, B));
    }
